    ...
    ui(new Ui::qrobot)
{
    ...
    my_hsv_timer = new QTimer(this);

    QObject::connect(my_hsv_timer,SIGNAL(timeout()),this,SLOT(hsv_look()));
    my_hsv_timer->start(1000);
...
//    cv::imshow("Final", imgFinal);
    ...
    vVal = imgchan[2].at<uchar>(ui->sldY->value(),ui->sldX->value());

    cv::imshow("Thresholded Image", imgThresholded);
    imgFinal = imgOriginal + imgRad + imgCen + imgCirc;
    cv::imshow("Final", imgFinal);
}

void qrobot::hsv_look(){
    ui->txtH->setText(QString::number(vHue));
    ui->txtS->setText(QString::number(vSat));
    ui->txtV->setText(QString::number(vVal));
}
